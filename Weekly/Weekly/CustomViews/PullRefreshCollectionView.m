...
@interface PullRefreshCollectionView () {
...
- (void)updateView {
	...
	if (_pullEnabled) {
		if (loadingLabel) {
			loadingLabel.frame = CGRectMake(0, [self.collectionViewLayout collectionViewContentSize].height + 10, self.frame.size.width, 20);
		} else {
			loadingLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, [self.collectionViewLayout collectionViewContentSize].height + 10, self.frame.size.width, 20)];
...
- (void)refreshControlAction:(id)sender {
	_currentPageNum = 1;
	[_pullRefreshDelegate collectionViewBeginPullRefreshLoading:self];
}
...
- (void)didScroll {
	if (self.contentOffset.y + self.frame.size.height < [self.collectionViewLayout collectionViewContentSize].height + kLoadingHeight && self.contentOffset.y > 0) {
		self.state = PullRefreshNormal;
	} else if (self.contentOffset.y + self.frame.size.height > [self.collectionViewLayout collectionViewContentSize].height + kLoadingHeight) {
...
- (void)didEndDragging {
	if (_currentPageNum < _totalPages && self.contentOffset.y > 0 &&
		self.contentOffset.y + self.frame.size.height > [self.collectionViewLayout collectionViewContentSize].height + kLoadingHeight &&
		_pullEnabled) {
		_currentPageNum++;
		[_pullRefreshDelegate collectionViewBeginPullRefreshLoading:self];
